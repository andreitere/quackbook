...
<template>
    <div
        class="transition-all duration-200 w-full flex h-auto flex-col space-y-2 group p-3 relative rounded border-[1px] border-solid  hover:shadow-md  focus-within:border-slate-300 focus-within:shadow-md" :class="[
            editMode ? ' border-solid border-slate-200 hover:shadow-md' : 'border-transparent',
        ]"
    >
        <div class="flex items-center gap-2">
            <EditorCellToolbar
                class="opacity-0 group-hover:opacity-100 top-3 right-3" :class="[
                    editMode ? '' : 'absolute',
                ]"
                ...
                @insert-above="$projects.insertCell('markdown', props.position, null)"
                @insert-below="$projects.insertCell('markdown', props.position + 1, null)"
            />
        </div>
        <div
            v-show="editMode"
